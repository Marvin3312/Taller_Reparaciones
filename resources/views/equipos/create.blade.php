        ...
        <form action="{{ route('equipos.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="tipo" name="tipo" required>
                    <option value="" {{ old('tipo') == '' ? 'selected' : '' }}>Seleccione un tipo</option>
                    <option value="Laptop" {{ old('tipo') == 'Laptop' ? 'selected' : '' }}>Laptop</option>
                    <option value="Smartphone" {{ old('tipo') == 'Smartphone' ? 'selected' : '' }}>Smartphone</option>
                ...
                <select class="form-control" id="id_marca" name="id_marca">
                    ...
                    @if (!empty($marcas))
                        @foreach ($marcas as $marca)
                            <option value="{{ $marca['id_marca'] }}" {{ old('id_marca') == $marca['id_marca'] ? 'selected' : '' }}>{{ $marca['nombre'] }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
