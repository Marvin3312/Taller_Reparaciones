    ...
    <div class="container">
        ...
        <form action="{{ route('tecnicos.update', $tecnico['id_tecnico']) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="nombre" name="nombre" value="{{ old('nombre', $tecnico['nombre'] ?? '') }}" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control" id="correo" name="correo" value="{{ old('correo', $tecnico['correo'] ?? '') }}">
            </div>
            <div class="mb-3">
                ...
                <select class="form-control" id="id_usuario" name="id_usuario" required>
                    ...
                    @if (!empty($usuarios))
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario['id_usuario'] }}" {{ old('id_usuario', $tecnico['id_usuario'] ?? '') == $usuario['id_usuario'] ? 'selected' : '' }}>
                                {{ $usuario['username'] }}
                            </option>
                        @endforeach
                    @endif
